...
public class FillDialog extends WPDialogWithPaintSelection implements Listener, MapSelectionListener {
    ...
    public FillDialog(App app, Dimension dimension, Layer[] layers, ColourScheme colourScheme, Integer[] biomes, CustomBiomeManager customBiomeManager, WorldPainterView view, ObservableBoolean selectionState) {
        ...
        InputMap inputMap = rootPane.getInputMap(WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
        inputMap.put(getKeyStroke(VK_Z, PLATFORM_COMMAND_MASK), "undo");
        rootPane.setDefaultButton(buttonFill);
    ...
    private void initClearLayer() {
        ...
        if (! layersInUse.isEmpty()) {
            ...
            if (radioButtonClearLayer.isSelected()) {
                buttonGroup1.clearSelection();
            }
            radioButtonClearLayer.setEnabled(false);
            comboBoxClearLayer.setModel(new DefaultComboBoxModel<>());
        }
    }
    ...
    private void fill() {
        ...
        try {
            Dimension result = ProgressDialog.executeTask(this, new ProgressTask<>() {
                @Override
                public String getName() {
                    if (radioButtonTerrain.isSelected()) {
                        return format("ui.progress.fillWith", ((Terrain) comboBoxTerrain.getSelectedItem()).getName());
                    } else if (radioButtonSetLayer.isSelected()) {
                        return format("ui.progress.fillWith", ((Layer) comboBoxSetLayer.getSelectedItem()).getName());
                    } else if (radioButtonClearLayer.isSelected()) {
                        return format("ui.progress.clearLayer", ((Layer) comboBoxClearLayer.getSelectedItem()).getName());
                    } else if (radioButtonInvertLayer.isSelected()) {
                        return format("ui.progress.invertLayer", ((Layer) comboBoxInvertLayer.getSelectedItem()).getName());
                    } else if (radioButtonBiome.isSelected()) {
                        return format("ui.progress.fillWith", biomeHelper.getBiomeName((Integer) comboBoxBiome.getSelectedItem()));
                    } else if (radioButtonResetBiomes.isSelected()) {
                        return WPI18n.s("ui.progress.resetBiomes");
                    } else if (radioButtonResetWater.isSelected()) {
                        return WPI18n.s("ui.progress.resetWaterLava");
                    } else if (radioButtonResetTerrain.isSelected()) {
                        return WPI18n.s("ui.progress.resetTerrain");
                    } else if (radioButtonMakeBiomesPermanent.isSelected()) {
                        return WPI18n.s("ui.progress.makeBiomesPermanent");
                    } else if (radioButtonAddToSelection.isSelected()) {
                        return WPI18n.s("ui.progress.addToSelection");
                    } else if (radioButtonRemoveFromSelection.isSelected()) {
                        return WPI18n.s("ui.progress.removeFromSelection");
                    } else if (radioButtonTerrainHeight.isSelected()) {
                        return getSelectedTerrainHeightOperation().getProgressName(spinnerTerrainHeight.getValue());
                    } else {
            ...
            if (result == null) {
                ...
                if (! checkBoxKeepOpen.isSelected()) {
                    ...
                    buttonCancel.setText(org.pepsoft.worldpainter.WPI18n.s("wp.close.d3d2e61733"));
                    initClearLayer();
                    setControlStates();
...
chunks:         for (int chunkX = 0; chunkX < TILE_SIZE; chunkX += 16) {
    ...
    private void changeTerrainHeight(ProgressReceiver progressReceiver) throws OperationCancelled {
        dimension.visitTilesForEditing().forFilter(filter).andDo(tile -> {
            final int worldTileX = tile.getX() << TILE_SIZE_BITS;
            final int worldTileY = tile.getY() << TILE_SIZE_BITS;
            final float selectedHeight = (Integer) spinnerTerrainHeight.getValue();
            final boolean filterSet = filter != null;
            switch (getSelectedTerrainHeightOperation()) {
                case RAISE_TO:
                    for (int x = 0; x < TILE_SIZE; x++) {
                        for (int y = 0; y < TILE_SIZE; y++) {
                            ...
                            if (strength > 0.0f) {
                                ...
                                if (currentHeight < selectedHeight) {
                                    tile.setHeight(x, y, (strength < 1.0f)
                                            ? (strength * selectedHeight) + ((1.0f - strength) * currentHeight)
                                            : selectedHeight);
                                }
                            }
                        }
                ...
                case RAISE_BY:
                    ...
                    for (int x = 0; x < TILE_SIZE; x++) {
                        for (int y = 0; y < TILE_SIZE; y++) {
                            ...
                            if (strength > 0.0f) {
                                final float currentHeight = tile.getHeight(x, y);
                                final float targetHeight = Math.min(currentHeight + selectedHeight, maxZ);
                ...
                case LOWER_TO:
                    ...
                    break;
                case LOWER_BY:
                    final int minZ = dimension.getMinHeight();
                    for (int x = 0; x < TILE_SIZE; x++) {
                        for (int y = 0; y < TILE_SIZE; y++) {
                            final float strength = filterSet
                                    ? filter.modifyStrength(worldTileX | x, worldTileY | y, 1.0f)
                            ...
                            if (strength > 0.0f) {
                                final float currentHeight = tile.getHeight(x, y);
                                final float targetHeight = Math.max(currentHeight - selectedHeight, minZ);
                                if (currentHeight > targetHeight) {
                                    tile.setHeight(x, y, (strength < 1.0f)
                                            ? (strength * targetHeight) + ((1.0f - strength) * currentHeight)
                                            : targetHeight);
                ...
                case SET_TO:
                    for (int x = 0; x < TILE_SIZE; x++) {
                        for (int y = 0; y < TILE_SIZE; y++) {
                            ...
                            if (strength > 0.0f) {
                                tile.setHeight(x, y, (strength < 1.0f)
                                        ? (strength * selectedHeight) + ((1.0f - strength) * tile.getHeight(x, y))
                                        : selectedHeight);
                            }
                        }
                    }
    ...
    private TerrainHeightOperation getSelectedTerrainHeightOperation() {
        final TerrainHeightOperation operation = (TerrainHeightOperation) comboBoxTerrainOperation.getSelectedItem();
        return (operation != null) ? operation : TerrainHeightOperation.SET_TO;
    }

    private static String format(String key, Object... args) {
        return MessageFormat.format(WPI18n.s(key), args);
    }
    ...
    private void initComponents() {
        ...
        setTitle(org.pepsoft.worldpainter.WPI18n.s("ui.t.global_operations"));

        jLabel1.setText(org.pepsoft.worldpainter.WPI18n.s("wp.perform.a.global.operation.69f41f4ae0"));
        ...
        buttonFill.setText(org.pepsoft.worldpainter.WPI18n.s("wp.go.5f075ae3e1"));
        ...
        radioButtonSetLayer.setText(org.pepsoft.worldpainter.WPI18n.s("wp.fill.with.layer.87407e4caf"));
        ...
        radioButtonResetBiomes.setText(org.pepsoft.worldpainter.WPI18n.s("wp.reset.biomes.to.automatic.5449fbcc1e"));
        ...
        radioButtonClearLayer.setText(org.pepsoft.worldpainter.WPI18n.s("wp.remove.a.layer.658bb479ee"));
        ...
        radioButtonResetTerrain.setText(org.pepsoft.worldpainter.WPI18n.s("wp.reset.terrain.and.layers.69026b8230"));
        radioButtonResetTerrain.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.reset.the.terrain.type.5d538c7bb3"));
        ...
        radioButtonTerrain.setText(org.pepsoft.worldpainter.WPI18n.s("wp.fill.with.terrain.type.cd5a6a097b"));
        ...
        radioButtonResetWater.setText(org.pepsoft.worldpainter.WPI18n.s("wp.reset.all.water.or.7364ba7af4"));
        radioButtonResetWater.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.this.resets.the.fluid.62f10a3a64"));
        ...
        radioButtonInvertLayer.setText(org.pepsoft.worldpainter.WPI18n.s("wp.invert.a.layer.f5e4500802"));
        ...
        radioButtonBiome.setText(org.pepsoft.worldpainter.WPI18n.s("wp.fill.with.biome.3f259e9cbf"));
        ...
        radioButtonMakeBiomesPermanent.setText(org.pepsoft.worldpainter.WPI18n.s("wp.make.automatic.biomes.permanent.fb4c11ab7b"));
        ...
        radioButtonAddToSelection.setText(org.pepsoft.worldpainter.WPI18n.s("wp.add.to.selection.3a9dd79925"));
        ...
        radioButtonRemoveFromSelection.setText(org.pepsoft.worldpainter.WPI18n.s("wp.remove.from.selection.cf88908864"));
        ...
        buttonFillTerrainSelectOnMap.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.select.a.terrain.type.a3d4782e3b"));
        ...
        buttonFillLayerSelectOnMap.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.select.a.layer.from.6223517dfb"));
        ...
        buttonRemoveLayerSelectOnMap.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.select.a.layer.from.6223517dfb"));
        ...
        buttonInvertLayerSelectOnMap.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.select.a.layer.from.6223517dfb"));
        ...
        buttonFillBiomeSelectOnMap.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.select.a.biome.from.df7d2e84ac"));
        ...
        comboBoxTerrainOperation.setModel(new javax.swing.DefaultComboBoxModel<>(TerrainHeightOperation.values()));
        ...
        jLabel3.setText(org.pepsoft.worldpainter.WPI18n.s("wp.html.u.change.height.2adbd06d3b"));
        jLabel3.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.raise.or.lower.the.a88950f4e0"));
        ...
        jLabel5.setText(org.pepsoft.worldpainter.WPI18n.s("wp.html.u.rotate.u.7821bfad84"));
        jLabel5.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.rotate.the.entire.dimension.7e513de31f"));
        ...
        jLabel7.setText(org.pepsoft.worldpainter.WPI18n.s("wp.html.u.shift.u.c7d202d3f8"));
        jLabel7.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.shift.the.entire.dimension.c116de4999"));
        ...
        jLabel2.setText(org.pepsoft.worldpainter.WPI18n.s("wp.other.global.tools.678ab6a36f"));

        checkBoxKeepOpen.setText(org.pepsoft.worldpainter.WPI18n.s("wp.keep.this.window.open.0c47528eeb"));
        checkBoxKeepOpen.setHorizontalTextPosition(javax.swing.SwingConstants.LEADING);

        jLabel8.setText("-");

        jLabel9.setForeground(new java.awt.Color(0, 0, 255));
        jLabel9.setText(org.pepsoft.worldpainter.WPI18n.s("wp.html.u.scale.u.87a47dabb4"));
        jLabel9.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.scale.the.entire.dimension.4b994ce695"));
        ...
        buttonUndo.setText(org.pepsoft.worldpainter.WPI18n.s("wp.undo.1cdc076b28"));
        buttonUndo.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.undo.the.most.recent.2dc12ea694"));
    ...
    private javax.swing.JComboBox<TerrainHeightOperation> comboBoxTerrainOperation;
    ...
    private enum TerrainHeightOperation {
        RAISE_TO("ui.fill.terrainHeight.raiseTo", "ui.progress.terrainHeight.raiseTo"),
        RAISE_BY("ui.fill.terrainHeight.raiseBy", "ui.progress.terrainHeight.raiseBy"),
        SET_TO("ui.fill.terrainHeight.setTo", "ui.progress.terrainHeight.setTo"),
        LOWER_TO("ui.fill.terrainHeight.lowerTo", "ui.progress.terrainHeight.lowerTo"),
        LOWER_BY("ui.fill.terrainHeight.lowerBy", "ui.progress.terrainHeight.lowerBy");

        TerrainHeightOperation(String labelKey, String progressKey) {
            this.labelKey = labelKey;
            this.progressKey = progressKey;
        }

        String getProgressName(Object value) {
            return format(progressKey, value);
        }

        @Override
        public String toString() {
            return WPI18n.s(labelKey);
        }

        private final String labelKey, progressKey;
    }
