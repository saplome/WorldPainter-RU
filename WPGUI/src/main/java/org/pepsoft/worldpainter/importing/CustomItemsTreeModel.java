...
public class CustomItemsTreeModel implements TreeModel {
    public CustomItemsTreeModel(World2 world, ItemType itemType) {
        ...
        if ((itemType == ItemType.ALL) || (itemType == ItemType.TERRAIN)) {
            ...
            if (! customTerrains.isEmpty()) {
                childrenOfRoot.add(TreeNode.TERRAINS);
            }
        }
        ...
        if (! customLayers.isEmpty()) {
            childrenOfRoot.add(TreeNode.LAYERS);
        }
        if (! customBiomes.isEmpty()) {
            childrenOfRoot.add(TreeNode.BIOMES);
        }
    }
    ...
    public List<Object> getSelectedItems(TreePath[] selectedTreePaths) {
        ...
        for (TreePath selectedPath: selectedTreePaths) {
            ...
            if (object == TreeNode.ROOT) {
                selectedItems.addAll(customTerrains);
                selectedItems.addAll(customLayers);
                selectedItems.addAll(customBiomes);
            } else if (object == TreeNode.TERRAINS) {
                selectedItems.addAll(customTerrains);
            } else if (object == TreeNode.LAYERS) {
                selectedItems.addAll(customLayers);
            } else if (object == TreeNode.BIOMES) {
                selectedItems.addAll(customBiomes);
    ...
    public Object getChild(Object parent, int index) {
        if (parent == TreeNode.ROOT) {
            return childrenOfRoot.get(index);
        } else if (parent == TreeNode.TERRAINS) {
            return customTerrains.get(index);
        } else if (parent == TreeNode.LAYERS) {
            return customLayers.get(index);
        } else if (parent == TreeNode.BIOMES) {
    ...
    public boolean isLeaf(Object node) {
        return ! ((node == TreeNode.ROOT) || (node == TreeNode.TERRAINS) || (node == TreeNode.LAYERS) || (node == TreeNode.BIOMES));
    }
    ...
    private final List<TreeNode> childrenOfRoot = new ArrayList<>();

    public enum TreeNode {
        ROOT("ui.import.customItems.root"),
        LAYERS("ui.import.customItems.layers"),
        TERRAINS("ui.import.customItems.terrains"),
        BIOMES("ui.import.customItems.biomes");
