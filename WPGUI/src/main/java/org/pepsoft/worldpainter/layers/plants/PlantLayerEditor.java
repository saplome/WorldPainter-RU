...
public class PlantLayerEditor extends AbstractLayerEditor<PlantLayer> {
    ...
    public PlantLayer createLayer() {
        return new PlantLayer(org.pepsoft.worldpainter.WPI18n.s("plant.layer.default"), org.pepsoft.worldpainter.WPI18n.s("plant.layer.default.desc"), Color.GREEN);
    }
        ...
        for (int i = 0; i < ALL_PLANTS.length; i++) {
            ...
            if (! isCompatibleWithPlatform(ALL_PLANTS[i], platform)) {
                ...
                if ((settings != null) && (settings.occurrence > 0)) {
                    ...
                    plantLabels[i].setText("<html><s>" + org.pepsoft.worldpainter.WPI18n.plant(ALL_PLANTS[i].getName()) + "</s></html>");
                } else {
                    plantLabels[i].setEnabled(false);
                    plantLabels[i].setText(org.pepsoft.worldpainter.WPI18n.plant(ALL_PLANTS[i].getName()));
                }
                plantLabels[i].setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.this.plant.is.not.52ff1ead73"));
    ...
    private void initPlantControls() {
        ...
        addCategory(panel, org.pepsoft.worldpainter.WPI18n.s("plant.cat.flowers"), 0, 18, false, PLANTS_AND_FLOWERS);
        addFiller(panel);
        ...
        addCategory(panel, org.pepsoft.worldpainter.WPI18n.s("plant.cat.various"), MUSHROOMS, CACTUS, SUGAR_CANE, FLOATING_PLANTS, END);
        addFiller(panel);
        panel = new JPanel(new GridBagLayout());
        panelPlantControls.add(panel);
        addCategory(panel, org.pepsoft.worldpainter.WPI18n.s("plant.cat.water"), WATER_PLANTS, DRIPLEAF);
        addCategory(panel, NETHER);
        addCategory(panel, org.pepsoft.worldpainter.WPI18n.s("plant.cat.hanging"), org.pepsoft.worldpainter.WPI18n.s("plant.cat.hanging.sub"), HANGING_DRY_PLANTS, HANGING_WATER_PLANTS);
        addFiller(panel);
    ...
    private void addPlantRow(final JPanel panel, final Plant plant, final int index, final boolean enabled) {
        ...
        synchronized (icons) {
            ...
            if (icon != null) {
                plantLabels[index] = new JLabel(org.pepsoft.worldpainter.WPI18n.plant(plant.getName()), new ImageIcon(icon), JLabel.TRAILING);
            } else {
                plantLabels[index] = new JLabel(org.pepsoft.worldpainter.WPI18n.plant(plant.getName()));
            }
        }
        ...
        if (! enabled) {
            ...
            spinners[index].setToolTipText(org.pepsoft.worldpainter.WPI18n.s("wp.this.plant.is.not.52ff1ead73"));
        }
    ...
    private void initComponents() {
        ...
        buttonClear.setText(org.pepsoft.worldpainter.WPI18n.s("wp.clear.dc30bc0c79"));
        ...
        checkBoxGenerateTilledDirt.setText(org.pepsoft.worldpainter.WPI18n.s("wp.turn.grass.and.dirt.1936040fee"));
        checkBoxGenerateTilledDirt.setEnabled(false);

        fieldName.setColumns(20);
        fieldName.setText("jTextField1");

        jLabel2.setText(org.pepsoft.worldpainter.WPI18n.s("wp.paint.d65951a421"));

        checkBoxOnlyValidBlocks.setSelected(true);
        checkBoxOnlyValidBlocks.setText(org.pepsoft.worldpainter.WPI18n.s("wp.only.place.on.valid.4d01455b1b"));
    ...
    private void checkBoxOnlyValidBlocksActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_checkBoxOnlyValidBlocksActionPerformed
        if (! checkBoxOnlyValidBlocks.isSelected()) {
            ...
            if ((exportSettings == null) || ((exportSettings instanceof JavaExportSettings) && ((JavaExportSettings) exportSettings).isRemovePlants())) {
                DesktopUtils.beep();
                showInfo(SwingUtilities.windowForComponent(this),
                        org.pepsoft.worldpainter.WPI18n.s("ui.plants.removeReminder.message"),
                        org.pepsoft.worldpainter.WPI18n.s("ui.plants.removeReminder.title"));
            }
        }
