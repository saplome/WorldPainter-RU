...
public class BiomesPanel extends JPanel implements CustomBiomeManager.CustomBiomeListener {
    ...
    private void initComponents() {
        ...
        addCustomBiomeButton.setToolTipText(org.pepsoft.worldpainter.WPI18n.s("ui.action.addACustomBiome"));
        addCustomBiomeButton.addActionListener(e -> {
            ...
            if (id == -1) {
                JOptionPane.showMessageDialog(parent, org.pepsoft.worldpainter.WPI18n.s("ui.message.maxCustomBiomesReached"), org.pepsoft.worldpainter.WPI18n.s("ui.dialog.maximumReached.title"), JOptionPane.ERROR_MESSAGE);
                return;
            }
        ...
        if (biomesSet != null) {
            ...
            for (final int biome: biomesSet.biomeOrder) {
                if (biome != -1) {
                    ...
                    tooltip.append(biomesSet.displayName(biome));
                    if (showIds) {
                        tooltip.append(" (");
                        ...
                        if (! options.isEmpty()) {
                            tooltip.append(" (");
                            tooltip.append(org.pepsoft.worldpainter.WPI18n.s("ui.biome.options"));
                            tooltip.append(": ");
    ...
    private String createOptionName(BiomeOption option) {
        final String key = "biome.option." + option.name();
        final String translated = org.pepsoft.worldpainter.WPI18n.s(key);
    ...
    private void addButton(CustomBiome customBiome) {
        ...
        button.setMargin(App.BUTTON_INSETS);
        button.setToolTipText(customBiome.getName() + " (" + biome + org.pepsoft.worldpainter.WPI18n.s("ui.tooltip.biomeRightClickOptions"));
    ...
    private Set<BiomeOption> findAvailableOptions(int baseId) {
        if ((biomesSet != null) && biomesSet.hasBiome(baseId)) {
    ...
    static class BiomesSet {
        BiomesSet(int[] biomeOrder, Set<BiomeDescriptor> descriptors, String[] sourceNames) {
            this.biomeOrder = biomeOrder;
            this.descriptors = descriptors;
            this.sourceNames = sourceNames;
        }

        boolean hasBiome(int biome) {
            return (biome >= 0) && (biome < sourceNames.length) && (sourceNames[biome] != null);
        }

        String displayName(int biome) {
            if (! hasBiome(biome)) {
                return null;
            }
            return org.pepsoft.worldpainter.WPI18n.biome(sourceNames[biome]);
        }
