...
//    }
    ...
    public static synchronized Configuration load() throws IOException, ClassNotFoundException {
        ...
        if (! configFile.isFile()) {
            File legacyConfigFile = new File(getLegacyConfigDir(), "config");
            if (legacyConfigFile.isFile()) {
                if (shouldMigrateLegacyConfiguration(legacyConfigFile)) {
    ...
    public static File getConfigDir() {
        ...
        if (SystemUtils.isMac()) {
            return new File(System.getProperty("user.home"), "Library/Application Support/WorldPainter Languages" + ((classifier != null) ? (" [" + classifier.toUpperCase() + "]") : ""));
        } else if (SystemUtils.isWindows()) {
            final String appDataStr = System.getenv("APPDATA");
            if (appDataStr != null) {
                return new File(appDataStr, "WorldPainter Languages" + ((classifier != null) ? (" [" + classifier.toUpperCase() + "]") : ""));
            }
        }
        // Backwards compatibility with existing installations:
        final File defaultDir = new File(HOME, ".worldpainter-languages" + ((classifier != null) ? ("-" + classifier.toLowerCase()) : ""));
        if (defaultDir.isDirectory()) {
            return defaultDir;
        }
        return new File(XDG_DATA_HOME, "worldpainter-languages" + ((classifier != null) ? ("-" + classifier.toLowerCase()) : ""));
    }
    ...
    /**
     * The configuration directory used by the original WorldPainter (and by releases of this fork prior to the
     * WorldPainter Languages rebranding). Only used to migrate existing settings to the new location.
     */
    private static File getLegacyConfigDir() {
        String classifier = System.getProperty("org.pepsoft.worldpainter.classifier");
        if (classifier != null) {
            classifier = classifier.trim();
            if (classifier.isEmpty()) {
    ...
    /**
     * Asks the user whether the settings from an existing regular WorldPainter installation should be copied to
     * WorldPainter Languages. If the settings appear to have been created by an incompatible (probably newer)
     * version of WorldPainter, a warning is shown instead of a plain question. In headless mode no dialog can be
     * shown; the settings are then copied without asking, but only if they are known to be compatible.
     */
    private static boolean shouldMigrateLegacyConfiguration(File legacyConfigFile) {
        Configuration legacyConfig = null;
        int legacyVersion = -1;
        try {
            legacyConfig = load(legacyConfigFile);
            // getVersion() is normalised to CURRENT_VERSION during deserialisation; getPreviousVersion() reports the
            // version that was actually stored on disk
            legacyVersion = legacyConfig.getPreviousVersion();
        } catch (IOException | ClassNotFoundException | Error | RuntimeException e) {
            logger.warn("Could not read legacy configuration to determine its version; assuming it is incompatible", e);
        }
        final boolean compatible = (legacyVersion >= 0) && (legacyVersion <= CURRENT_VERSION);
        if (java.awt.GraphicsEnvironment.isHeadless()) {
            // We cannot ask the user; only copy the settings if they are known to be compatible
            return compatible;
        }
        installStartupLookAndFeel(legacyConfig);
        final String message = compatible
                ? WPI18n.s("ui.dialog.migrateSettings.message")
                : (WPI18n.s("ui.dialog.migrateSettings.message") + "\n\n" + WPI18n.s("ui.dialog.migrateSettings.versionWarning"));
        final int messageType = compatible ? javax.swing.JOptionPane.QUESTION_MESSAGE : javax.swing.JOptionPane.WARNING_MESSAGE;
        return javax.swing.JOptionPane.showConfirmDialog(null, message, WPI18n.s("ui.dialog.migrateSettings.title"),
                javax.swing.JOptionPane.YES_NO_OPTION, messageType) == javax.swing.JOptionPane.YES_OPTION;
    }

    /**
     * Installs the look and feel configured in the legacy configuration (or the system look and feel if there is
     * none), so that dialogs shown before the main application startup match the WorldPainter style. The regular
     * startup code will install the look and feel again later.
     */
    private static void installStartupLookAndFeel(Configuration legacyConfig) {
        try {
            ...
            logger.warn("Could not install look and feel for startup dialog", t);
        }
    }

    /**
     * Copies the settings from the legacy WorldPainter configuration directory to the new WorldPainter Languages
     * directory. The legacy directory is intentionally left untouched, since it may still be in use by an original
     * WorldPainter installation on the same system.
     */
    private static void migrateLegacyConfiguration() throws IOException {
        final File newConfigDir = getConfigDir();
        final File legacyConfigDir = getLegacyConfigDir();
        logger.info("Configuration found in legacy location (" + legacyConfigDir + "); copying it to new location (" + newConfigDir + ")");
        if (! newConfigDir.isDirectory()) {
            newConfigDir.mkdirs();
        }
        final File[] legacyContents = legacyConfigDir.listFiles((dir, name) -> {
            // Skip log files, since the new log file is already open in the new location
            return ! name.startsWith("logfile");
        });
        if (legacyContents == null) {
            return;
        }
        for (File legacyFile: legacyContents) {
            if (legacyFile.isDirectory()) {
                FileUtils.copyDir(legacyFile, new File(newConfigDir, legacyFile.getName()));
            } else if (legacyFile.isFile()) {
                FileUtils.copyFileToDir(legacyFile, newConfigDir);
            } else {
                logger.warn("Directory entry encountered which was neither file nor directory: " + legacyFile);
            }
        }
    }
