...
public abstract class AbstractWorldExporter implements WorldExporter {
    ...
    protected ChunkFactory.Stats parallelExportRegions(Dimension dimension, File worldDir, ProgressReceiver progressReceiver) throws OperationCancelled {
        if (progressReceiver != null) {
            progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.exportingDimension"), WPI18n.dimensionName(dimension)));
        }
        ...
        try {
            ...
            if (! abort.get()) {
                ...
                synchronized (fixups) {
                    if (! fixups.isEmpty()) {
                        if (progressReceiver != null) {
                            progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.remainingFixups"), WPI18n.dimensionName(dimension)));
    ...
    protected ExportResults firstPass(MinecraftWorld minecraftWorld, Dimension dimension, Point regionCoords, Map<Point, Tile> tiles, boolean tileSelection, Map<Layer, LayerExporter> exporters, ChunkFactory chunkFactory, ProgressReceiver progressReceiver) throws OperationCancelled {
        ...
        if (progressReceiver != null) {
            if (ceiling) {
                progressReceiver.setMessage(WPI18n.s("ui.export.progress.generatingCeiling"));
            } else {
                progressReceiver.setMessage(WPI18n.s("ui.export.progress.generatingLandscape"));
            }
        }
            ...
            for (Layer layer: secondaryPassLayers) {
                ...
                if (progressReceiver != null) {
                    if (minecraftWorld instanceof InvertedWorld) {
                        progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.exportingLayerForCeiling"), layer.getName(), getStageName(stage)));
                    } else {
                        progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.exportingLayer"), layer.getName(), getStageName(stage)));
                    }
                }
    ...
    protected void blockPropertiesPass(MinecraftWorld minecraftWorld, Point regionCoords, BlockBasedExportSettings exportSettings, ProgressReceiver progressReceiver) throws OperationCancelled {
        ...
        final String nouns = getBlockPropertyNouns(exportSettings);
        if (progressReceiver != null) {
            progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.calculatingInitial"), nouns));
        }
        ...
        if (lowMark != Integer.MAX_VALUE) {
            if (progressReceiver != null) {
                progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.propagating"), nouns));
            }
    ...
    protected ExportResults exportRegion(MinecraftWorld minecraftWorld,
        ...
        return doWithMdcContext(() -> {
            if (progressReceiver != null) {
                progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.exportingRegion"), regionCoords.x, regionCoords.y, WPI18n.dimensionName(dimension)));
            }
    ...
    protected void performFixups(final File worldDir, final Dimension dimension, final ChunkFactory.Stats stats,
        ...
        try {
            ...
            for (Entry<Point, List<Fixup>> entry: fixups.entrySet()) {
                if (progressReceiver != null) {
                    progressReceiver.setMessage(MessageFormat.format(WPI18n.s("ui.export.progress.performingFixups"), entry.getKey().x, entry.getKey().y));
                }
    ...
    private String getStageName(SecondPassLayerExporter.Stage stage) {
        switch (stage) {
            case CARVE:
                return WPI18n.s("ui.export.stage.carve");
            case ADD_FEATURES:
                return WPI18n.s("ui.export.stage.addFeatures");
            default:
                return stage.name().toLowerCase();
        }
    }

    private String getBlockPropertyNouns(BlockBasedExportSettings exportSettings) {
        final boolean lighting = exportSettings.isCalculateSkyLight() || exportSettings.isCalculateBlockLight();
        final boolean leafDistances = exportSettings.isCalculateLeafDistance();
        if (lighting && leafDistances) {
            return WPI18n.s("ui.export.blockProperties.lightingAndLeafDistances");
        } else if (lighting) {
            return WPI18n.s("ui.export.blockProperties.lighting");
        } else if (leafDistances) {
            return WPI18n.s("ui.export.blockProperties.leafDistances");
        } else {
            return WPI18n.s("ui.export.blockProperties.none");
        }
    }
