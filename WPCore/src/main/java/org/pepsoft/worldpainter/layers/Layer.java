...
public abstract class Layer implements Serializable, Comparable<Layer> {
    ...
    public String getName() {
        return getLocalisedProperty("name", name);
    }
    ...
    public String getDescription() {
        return getLocalisedProperty("desc", description);
    }

    private String getLocalisedProperty(String suffix, String fallback) {
        final String simpleName = getClass().getSimpleName();
        if (! simpleName.isEmpty()) {
            final String classKey = "layer." + simpleName + "." + suffix;
            final String classValue = WPI18n.s(classKey);
            if (! classValue.equals(classKey)) {
                return classValue;
            }
        }
        if (id != null) {
            final String idKey = "layer." + id + "." + suffix;
            final String idValue = WPI18n.s(idKey);
            if (! idValue.equals(idKey)) {
                return idValue;
            }
        }
        return fallback;
