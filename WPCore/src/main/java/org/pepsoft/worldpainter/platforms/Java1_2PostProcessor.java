...
public class Java1_2PostProcessor extends PostProcessor {
    ...
    public void postProcess(MinecraftWorld minecraftWorld, Box volume, ExportSettings exportSettings, ProgressReceiver progressReceiver) throws ProgressReceiver.OperationCancelled {
        ...
        if (progressReceiver != null) {
            progressReceiver.setMessage(WPI18n.s("ui.export.progress.postProcessingFirstPass"));
        }
...
//                }
                ...
                for (int z = minZ; z <= columnMaxZ; z++) {
                    ...
                    if (! leavePlants) {
                            ...
                            switch (blockType) {
                                ...
                                case BLK_LARGE_FLOWERS:
                                    if ((data & 0x8) == 0) {
                                        ...
                                        if ((blockTypeBelow != BLK_GRASS) && (blockTypeBelow != BLK_DIRT)) {
                                            // Double high plants can (presumably) only exist on grass or dirt
                                            // TODO: check
                                            if (traceEnabled) {
                                ...
                                case BLK_CHORUS_PLANT:
                                    if ((blockTypeBelow != BLK_END_STONE) && (blockTypeBelow != BLK_CHORUS_PLANT)) {
                                        // Chorus flower and plant blocks can only be on top of end stone or other
                                        // chorus plant blocks
                                        minecraftWorld.setMaterialAt(x, y, z, Material.AIR);
                                        blockType = BLK_AIR;
                                        data = 0;
                        ...
                        if (blockType == BLK_LARGE_FLOWERS) {
                            if ((data & 0x8) == 0x8) {
                                ...
                                // of the correct type because the combo was already checked when the lower half was
                                // encountered in the previous iteration
                                if (blockTypeBelow != BLK_LARGE_FLOWERS) {
                                    // There's a non-double high plant block below; replace this block with air
                                    if (traceEnabled) {
                                ...
                                if (blockTypeAbove == BLK_LARGE_FLOWERS) {
                                    if ((minecraftWorld.getDataAt(x, y, z + 1) & 0x8) == 0) {
                                        // There's another lower half above. Replace this block with air
                                        if (traceEnabled) {
        ...
        if (flowWater || flowLava) {
            if (progressReceiver != null) {
                progressReceiver.setMessage(WPI18n.s("ui.export.progress.postProcessingFluidsPass"));
            }
